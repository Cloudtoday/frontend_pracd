...
class Dashboard extends Component {
  ...
  submitProfile = async (e) => {
    ...
    const payload = {
      ...
      doctor_description: profile.doctor_description,
      fees: profile.fees ? Number(profile.fees) : undefined,
    };
                  ...
                  {this.state.loading ? (
                    ...
                    <Form onSubmit={this.submitProfile}>
                      <Row className="g-3">
                        ...
                        <Col md={12}><Form.Group><Form.Label>Address</Form.Label><Form.Control name="address" value={this.state.profile?.address || ''} onChange={this.handleProfileChange} /></Form.Group></Col>
  ...
  renderPatientLayout(user) {
    ...
    const nextDateStr = nextDate ? new Date(nextDate).toLocaleDateString() : '-';
    return (
      <Container className="py-4">
        <h2 className="fw-semibold">Dashboard</h2>
        <p className="text-muted">Home / Dashboard</p>
            ...
            <div>
              <Row className="g-3">
                {[
                  ...
                  { label: 'Date', value: (x) => (x.date ? new Date(x.date).toLocaleString() : '-') },
                  ...
                  ...(isAdmin ? [
                    { label: 'Booking Type', value: (x) => x.booking_type || x.type || x.mode || '-' },
                  ] : [])
                ].map((item, idx) => {
